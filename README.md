...
- **🔄 Проверить** — на время проверки сообщение меняется на `⏳ Проверяю «сайт»…`, а потом
  над карточкой появляется строка с результатом: `✅ Проверено 15:52:05 — 200 OK за 0.07 с`,
  `❌ Проверено … — HTTP 502` или `❌ Проверено … — connection refused`. Так видно, что кнопка
  сработала, даже если статус не изменился.
